...
def low_d(upstr):
    ...
    for i in range(0,len(upstr)):
        a[i]=upstr[i]
        if upstr[i]>='A' and upstr[i]<='Z':
            upstr[i]=upstr[i].lower()
    return a


def course_select(num, c):
    if len(num)==0:
        return True
    c_name=c.display_name_with_default
    c_name=c_name.lower()
    ch_words=get_ch(num)
    eg_words=get_eg(num)
    for word in ch_words:
        if re.search(word,c_name):
            return True
    for word in eg_words:
        if re.search(word,c_name):
            return True
    return False
...
def cat_select(c_name, c):
    if len(c_name)==0:
        return True
    if c_name==c.display_number_with_default[0:2]:
        return True
    return False
