class @HTMLEditingDescriptor
  ...
  #   tinyMCE incorrectly decides that the suffix should be "", which means it fails to load files.
      ...
      @tiny_mce_textarea = $(".tiny-mce", @element).tinymce({
        ...
        visual: false,
        plugins: "textcolor, link, image, codemirror, fileuploadselect, imageuploadselect",
        codemirror: {
          path: "#{baseUrl}/js/vendor"
        },
        image_advtab: true,
        # We may want to add "styleselect" when we collect all styles used throughout the LMS
        toolbar: "formatselect | fontselect | bold italic underline forecolor wrapAsCode | bullist numlist outdent indent blockquote | link unlink image | code | fuploadselect | iuploadselect",
